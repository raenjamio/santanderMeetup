...
/**
 * REST controller for managing {@link com.raenjamio.domain.Follow}.
 */
@RestController
@RequestMapping("/api")
public class FollowResource {
    ...
    public ResponseEntity<List<Follow>> getAllFollows(Pageable pageable) {
        ...
        Page<Follow> page = followService.findAll(pageable);
    ...
    @GetMapping("/follows/users/{idUser}")
    public ResponseEntity<List<Follow>> getAllFollowsByUser(Pageable pageable, @PathVariable String idUser) {
        log.debug("REST request to get a page of Follows");
        Page<Follow> page = followService.findAllByUser(pageable, idUser);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
