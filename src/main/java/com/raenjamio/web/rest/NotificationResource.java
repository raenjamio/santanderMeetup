...
/**
 * REST controller for managing {@link com.raenjamio.domain.Notification}.
 */
@RestController
@RequestMapping("/api")
public class NotificationResource {
    ...
    /**
     * {@code GET  /notifications/:id} : getAll notificacions by userId
     *
     * @param idUser the id of the user to retrieve.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the notification, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/users/{idUser}/notifications")
    public ResponseEntity<List<Notification>> getNotificationsByUser(@PathVariable String idUser) {
        log.debug("REST request to get Notification : {}", idUser);
        List<Notification> notification = notificationService.findAllByUser(idUser);
